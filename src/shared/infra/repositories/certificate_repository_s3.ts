...
export async function saveCertificate(
  ...
  try {
    ...
    const certificateUrl = `https://${process.env.BUCKET_NAME}.s3.${process.env.REGION}.amazonaws.com/CERTIFICADOS-PRINT/${userId}-${eventId}-certificado.pdf`;
...
export async function saveCertificateExternal(
  ...
  const params = {
    Bucket: `${process.env.BUCKET_NAME}`,
    Key: `CERTIFICADOS-EXTERNOS-PRINT/${email}-${eventId}-certificado.pdf`,
    Body: certificatePdf,
  };
  console.log(params.Key);
  try {
    ...
    const data = await s3.send(command);
    console.log("Certificate uploaded successfully. Data:", data);
    const certificateUrl = `https://${process.env.BUCKET_NAME}.s3.${process.env.REGION}.amazonaws.com/CERTIFICADOS-EXTERNOS-PRINT/${email}-${eventId}-certificado.pdf`;
    return certificateUrl;
  } catch (error: any) {
    console.error("Error uploading certificate to S3:", error);
    throw new Error(`Erro ao salvar certificado no S3: ${error.message}`);
